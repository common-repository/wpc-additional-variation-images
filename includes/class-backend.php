...
class Wpcvi_Backend {
	...
	function add_images_form( $loop, $variation_data, $variation ) {
		...
		foreach ( explode( ',', $ids ) as $attach_id ) {
			...
			if ( $attachment ) {
				echo '<li class="wpcvi-image" data-id="' . esc_attr( $attach_id ) . '"><span class="wpcvi-image-thumb"><a class="wpcvi-image-remove" href="#"></a><img src="' . esc_url( $attachment[0] ) . '" width="40" height="40"  alt=""/></span></li>';
			}
		}
